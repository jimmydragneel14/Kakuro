...
global jugada
global cont_nivel
cont_nivel=0#se inicializa contador en 0, para poder comenzar en el nivel 1
jugada=[]
audio_global=False
lista_top_uno=[]
lista_top_dos=[]
...
def leer_archivo_partidas():#funcion que nos permite el archivo en donde viene la estructura de las partidas
    ...
    global lista
    global partida
    global cont_nivel #variable o contador que permite al contador ir pasando de nivel de forma ordenada
    global kakuro
    global direccion
    ...
    if nivel.get()=="3 Neuronas":
        ...
        print(lista)
        celdas(lista)
    if nivel.get()=="Multinivel":#funcionalidad multinivel
        if cont_nivel==2:#si el contador llega a 2, se a al nivel de 3 neuronas
            partida=eval(lineas[2])#se saca la partida y se quitan los strings
            partida=partida[0]#se saca la primera tabla del nivel 3 neuronas
            cont_nivel=cont_nivel+1#se suma uno al contador para poder luego cambiar de nivel
            lista=partida# se define la nueva lista con la cual se podra revisar la tabla
            celdas(lista)# se crea a tabla con la funcion celdas
...
def borrar_direccion(direccion):
    return borrar_aux(direccion, 0,[])

def borrar_aux(direccion,cont,result):
    if cont==len(direccion)-1:
        return []
    else:
        return [direccion[cont].delete(0,END)] + borrar_aux(direccion,cont+1,result)
...
def jugar():#funcion donde se encuentra la tabla y botones del juego
    ...
    boton_borrar_jugada= Button(kakuro, text="Deshacer jugada",  fg= "black", bg="pink", width=2, height=1, font="Arial 12", padx=47, pady=6,state="disabled", command=borrar_jugada)
...
##    lista_boton=[boton1,boton2,boton3,boton4,boton5,boton6,boton7,boton8,boton9, boton_iniciar, boton_borrar_jugada, boton_terminar, boton_top,boton_borrar_juego]
    ...
    boton_cargar.place(x=480,y=660)
...
##                        j.config(bg="white")
    ...
    if cont2!=0:
        pass
    if cont==9:#condicion para cuando se termina la tabla del kakuro
...
def borrar_jugada():#funcion para borrar las jugadas
    global jugada
    global lista_jugada
    global contenido
    if len(jugada)==0:
        messagebox.showinfo("Aviso","No has ingresado ninguna jugada para borrar!!! >:v ")# se envia un avisoF

    ultima=jugada[len(jugada)-1]#se asigna a una variable, la utlima jugada 
    lista_jugada.append(ultima)#se agrega a esta lista que va a estar en la lista para rehacer jugada, esta va a tener la direccion de memoria del entry
    contenido.append(ultima.get())#esta lista va a guardar los valores obtenidos de los entrys
    ultima.delete(0,END)#se borra lo que haya en los entrys
    jugada.remove(ultima)#y se remuerva el entry o jugada de la lista borrar para poder borrar el proximo

def rehacer_jugada():#funcion para rehacer las jugadas realizadas
    ...
    jugada.append(lista_jugada[len(lista_jugada)-1])#se agrega a la lista para borrar la jugada, si se requiere
    contenido.pop()#se saca el ultimo elemento en "contenido"
    lista_jugada.remove(lista_jugada[len(lista_jugada)-1])#se remueve de la lista para ana;izar el siguiente 
...
def revisa_suma():# funcion que sirve para poder revisar si la suma de los digitos ingresados es igual que el numero clave
    ...
    for i in lista:
        ...
        forma=i[0]
        if forma == 2:
            for e in range(num_fila+1, num_fila + cantidad_espacios+1):#se recorre la matriz en forma de columna
                    valor_actual=lista_num[e][num_columna]#se define una variable como un valor de la matriz
                    if not isinstance(valor_actual,str)==True:#si no es string sucede esto
                        valor_actual.config(bg="white")#se configura que el entry es blanco
                        if valor_actual!=-1 and valor_actual!=0:#se confirma que no es un interger
                            if valor_actual.get()!="":#si el valor que se saca, no es un string vacio sucede esto
                                    contenedor.append(valor_actual.get())#se guarda en un contenedor
            if len(contenedor)==cantidad_espacios:#si la cantidad de elementos del contenedor es igual a la de cantidad de espacios, esto significa que ya se rellenaron todos los entrys
                for j in contenedor:#se procede a ejecutar la suma
                    result=result+float(j)
                if result!=numero:#si la suma es diferente del numero clave, se envia el mensaje respectivo
                    valor_actual.config(bg="red")
                    messagebox.showinfo("Aviso","La suma no concuerda con el numero clave")
            result=0#se resetean las variables
            contenedor=[]
                            
        if forma == 1:
            for e in range(num_columna+1, num_columna + cantidad_espacios+1):#se recorre la matriz en forma de fila
                    valor_actual=lista_num[num_fila][e]#se define una variable como un valor de la matriz
                    
                    if not isinstance(valor_actual,str)==True:
                        valor_actual.config(bg="white")
                        if valor_actual!=-1 and valor_actual!=0:
                                if valor_actual.get()!="":#si el contenido del entry es diferente de -1 o 0, se agrega al contenedor
                                    contenedor.append(valor_actual.get())
            if len(contenedor)==cantidad_espacios:#si el largo de contenedor es igual a la cantidad de espacios, esto significa que ya se llenaron todos los entrys
                for j in contenedor:#se procede a sumar el contenido
                    result=result+float(j)
                if result!=numero:#si la suma es incorrecta se envia el mensaje respectivo
                    valor_actual.config(bg="red")
                    messagebox.showinfo("Aviso","La suma no concuerda con el numero clave")
            result=0
            contenedor=[]
...
def cronometro():#funcion para medir el timepo
    global et_horas, et_minutos, et_segundos
    global segundos
    global minutos
    global horas
    ...
    minutos=0
    horas=0
    #se ponen etiquetas que se van ir renovando
    et_horas=Label(kakuro,bg="#FFFFFF",text=horas, fg="#CF4647", font="verdana 7", width=8)
    et_horas. place(x=50, y=680)
    et_minutos=Label(kakuro,bg="#FFFFFF",text=minutos, fg="#CF4647", font="verdana 7", width=8)
    et_minutos. place(x=120, y=680)
    ...
    while True:
        ...
        if segundos>=60:#si segundos es mayor o igual que 60, sucede esto:
            segundos=0#los segundos se resetean
            minutos=minutos+1#Se suma uno a minutos
            et_minutos.configure(text=minutos)#se configuran las etiquetas
            et_segundos.configure(text=segundos)
            et_segundos.update()#se actualizan
            et_minutos.update()
            
            if minutos>=60:#si segundos es mayor o igual que 60, sucede esto:
                minutos=0#los minutos se resetean
                horas=horas+1#se suma uno a horas
                et_minutos.configure(text=minutos)#se configuran las etiquetas
                et_horas.configure(text=horas)
                et_minutos.update()#se actualizan las etiquetas
                et_horas.update()
            
        time.sleep(1)#se define que este proceso va a durar 1 segundo por segundo
    return cronometro()
        
def hilo_tiempo():#funcion para que la funcion cronometro no perjudique el programa
    hilo=Thread(target=cronometro, args=())
    hilo.start()
    
def timer():#funcion para poner un tiempo limite
    global linea_horas
    global linea_minutos
    global linea_segundos
    global et_horas, et_minutos, et_segundos
    global horas, segundos, minutos
    global kakuro
    #se ponen etiquetas que se van ir actualizando
    et_horas=Label(kakuro,bg="#FFFFFF",text=horas, fg="#CF4647", font="verdana 7", width=8)
    et_horas. place(x=50, y=680)
    et_minutos=Label(kakuro,bg="#FFFFFF",text=minutos, fg="#CF4647", font="verdana 7", width=8)
    et_minutos. place(x=120, y=680)
    ...
    horas=int(horas)
    while True:#proceso para acortando el tiempo
        segundos=segundos-1#se va ir restando segundos
        et_segundos.configure(text=segundos)#se configura la etiqueta
        et_segundos.update()#se actualiza
        if segundos==0 and minutos==0 and segundos==0:#si todos son ceros, se marca un aviso
            answer=messagebox.askquestion("Aviso","Se acaba de terminar el tiempo ¿Desea continuar?")
            if answer=="yes":#si se contesta si, se da mas tiempo
                segundos=linea_segundos.get()
                segundos=int(segundos)
                minutos=linea_minutos.get()
                minutos=int(minutos)
                horas=linea_horas.get()
                horas=int(horas)
            if answer=="no":#si se dice no, se destruye el kakuro
                kakuro.destroy()
        if segundos==0 and minutos!=0:#si los segundos son 0, pero los minutos no
            segundos=59#se pone que segundos es 59
            minutos=minutos-1#se resta uno a minutos
            et_minutos.configure(text=minutos)# se configuran las etiquetas 
            et_segundos.configure(text=segundos)
            et_segundos.update()#se actualizan
            et_minutos.update()
            if minutos==0 and horas!=0:#si los minutos son 0 pero las horas no, sucede esot
                minutos=59#los minutos son 59
                et_minutos.configure(text=minutos)#se configura la etiqueta
                et_horas.configure(text=horas)
                et_minutos.update()#se actualiza
                horas=horas-1
            if minutos==0 and horas==0:#si los minutos y segundos son cero
                minutos=0#los minutos son 0
                segundos=59#los segundos son 59 por logica
                if horas!=0:
                    horas=horas-1
                    et_horas.configure(text=horas)#se configuran las etiquetas
                et_horas.update()#se actualiza
        
        time.sleep(1)#se define que va a durar un segundo cada ciclo
    return timer()
    ...
    for i in matriz:#se recorre la matriz 
        for casilla in i:
            ...
            if casilla=='':#si es un string vacio, es un entry normal
                ...
                lista_num[fila].append(x)#se va formando una matriz extra en donde hay direcciones de herramientas GUI
                columna=columna+1
            ...
            if  casilla!=-1 and casilla!='':#si no es un -1 o un string vacio, sucede esto:
                if n not in casilla:#si no tiene un ".", significa que es un entry que anteriormente tenia un digito
                    ...
                    x.bind('<Button-1>', (lambda x = x: callback(x)))#se enlazan los botones con la funcion callback
                    x.grid(column=columna,row=fila)
                    x.insert(0,casilla)#se inserta "casilla", por si habia algun elemento digitado anteriormente 
                    lista_num[fila].append(x)#se va
                    columna=columna+1
        
                if n in casilla:#si tiene un ".", significa que es un label con el numero clave 
                    x=Button(marco,text=casilla,bg="azure",fg="black",width=2, font="Arial 10", padx=8, pady=8)
                    x.grid(column=columna,row=fila)
                    columna=columna+1
                    lista_num[fila].append(casilla)#se va forma la matriz extra
